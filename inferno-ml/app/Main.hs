...
import Inferno.Module.Prelude (baseOpsTable, builtinModulesOpsTable, builtinModulesPinMap, builtinModulesTerms)
...
main = do
  ...
  let prelude = mlPrelude @IO @(MlValue ())
    ...
    Right (ast, _comments) ->
      if parse args
        then do
          ...
          putStrLn "Parsed Expr successfully"
        else do
